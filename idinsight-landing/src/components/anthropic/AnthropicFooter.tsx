const footerGroups = [
  {
    heading: "About",
    links: [
      { label: "Our people", href: "https://www.idinsight.org/about/our-people/" },
      { label: "Impact", href: "https://www.idinsight.org/about/impact/" },
      { label: "Careers", href: "https://www.idinsight.org/about/careers/" },
      { label: "News", href: "https://www.idinsight.org/about/news/" },
    ],
  },
  {
    heading: "Capabilities",
    links: [
      { label: "Impact evaluations", href: "https://www.idinsight.org/methodology/decision-focused-impact-evaluations/" },
      { label: "Data science", href: "https://www.idinsight.org/methodology/data-science/" },
      { label: "Monitoring systems", href: "https://www.idinsight.org/methodology/monitoring-systems/" },
      { label: "All services", href: "https://www.idinsight.org/services/" },
    ],
  },
  {
    heading: "Resources",
    links: [
      { label: "IDeas Blog", href: "https://www.idinsight.org/ideas-and-insights/blog/" },
      { label: "Publications", href: "https://www.idinsight.org/ideas-and-insights/publications-2/" },
      { label: "Impact Measurement Guide", href: "https://guide.idinsight.org/" },
      { label: "Tools", href: "https://www.idinsight.org/ideas-and-insights/tools/" },
    ],
  },
  {
    heading: "Connect",
    links: [
      { label: "Contact us", href: "https://www.idinsight.org/contact-us/" },
      { label: "Donate", href: "https://www.idinsight.org/give/" },
      { label: "Our offices", href: "https://www.idinsight.org/where-we-work/our-offices/" },
      { label: "Privacy policy", href: "https://www.idinsight.org/about/privacy-policy/" },
    ],
  },
];

const socialLinks = [
  {
    label: "Twitter",
    href: "https://twitter.com/IDinsight",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    label: "LinkedIn",
    href: "https://www.linkedin.com/company/idinsight/",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z" />
      </svg>
    ),
  },
  {
    label: "Facebook",
    href: "https://www.facebook.com/idinsight/",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
      </svg>
    ),
  },
  {
    label: "Instagram",
    href: "https://www.instagram.com/idinsight_official/",
    icon: (
      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
      </svg>
    ),
  },
];

export default function AnthropicFooter() {
  return (
    <footer className="bg-slate-dark text-ivory-light rounded-t-[2rem]">
      <div className="max-w-[1400px] mx-auto px-5 sm:px-8 lg:px-10 pt-16 pb-10">
        {/* Top: logo + newsletter CTA */}
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6 mb-12 pb-10 border-b border-ivory-light/10">
          <div>
            <span className="text-lg font-bold tracking-tight">
              IDinsight
            </span>
            <p className="text-sm text-ivory-light/50 mt-2 max-w-xs">
              Improving lives with data and evidence
            </p>
          </div>
          <a
            href="https://idinsight.us11.list-manage.com/subscribe?u=5eaborq=d9cda2c4f"
            className="inline-flex items-center px-5 py-2.5 rounded-full border border-ivory-light/20 text-sm text-ivory-light/80 hover:border-ivory-light/40 hover:text-ivory-light transition-colors"
          >
            Subscribe to our newsletter
          </a>
        </div>

        {/* Link groups */}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-8 lg:gap-12 mb-12">
          {footerGroups.map((group) => (
            <div key={group.heading}>
              <h3 className="text-xs font-medium tracking-wide uppercase text-ivory-light/40 mb-4">
                {group.heading}
              </h3>
              <ul className="space-y-3">
                {group.links.map((link) => (
                  <li key={link.label}>
                    <a
                      href={link.href}
                      className="text-sm text-ivory-light/70 hover:text-ivory-light transition-colors"
                    >
                      {link.label}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}
        </div>

        {/* Bottom: social + credits */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-8 border-t border-ivory-light/10">
          <div className="flex items-center gap-5">
            {socialLinks.map((social) => (
              <a
                key={social.label}
                href={social.href}
                target="_blank"
                rel="noopener noreferrer"
                className="text-ivory-light/40 hover:text-ivory-light transition-colors"
                title={social.label}
              >
                {social.icon}
              </a>
            ))}
          </div>
          <p className="text-xs text-ivory-light/30">
            Design and development by{" "}
            <a
              href="https://www.designbysoapbox.com/"
              target="_blank"
              rel="noopener noreferrer"
              className="hover:text-ivory-light/50 transition-colors"
            >
              Soapbox
            </a>
          </p>
        </div>
      </div>
    </footer>
  );
}
